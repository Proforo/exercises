...
- Review the starting HTML provided and notice the lists and sub-lists written in previous assignment have been simplified from three levels, down to two levels (simply the numbered tasks and their subtasks)


## Steps
1. Wrap the entirety of the content within each of the first-level `.todo` list-items (the item text, and the sub-list, if it exists) in a `<details>` element
   - For `.todo` items that do not have a sub-task lists, simply wrap the list text only
2. Within the new `<details>` element, wrap only the name of that task (the text that describes the task, for example "Take out the dog") in a `<summary>` element
   - **Observe**: The `<details>` element has become a functional dropdown, with the `<summary>` element as the clickable toggle for exposing the additional content within
...
4. To make styling within each `todo` element easier as you move forward, set the first one of the `<details>` blocks to be opened by default by adding an attribute of `open` to the `<details>` opening tag, giving it a value of `true`
   <!-- RP: Not sure why this was flagged as confusing, as it's just providing additional details to help them better understand. but if it's confusing, just remove the explanation and leave it as "just do this".
...
9. Add a new `<span>` element within each of the `<details>` task elements, placing it directly before the `<summary>` (this is now its first child element, with the `<summary>` directly following it)
   1. Between the opening and closing `<span>` tags, put the content: `&check;` 
   1. Test the page in your browser to ensure the `&check;` value converts to a checkmark character (`✓`)
      - This is an HTML "special character" shortcode that will convert to a non-standard character when rendered by the browser.
